	...
	$.fn.tagedit = function(options) {
		...
		*/
		function inputsToList() {
			...
			html += '<input type="text" name="'+baseName+'[]" value="" onfocus="hideInfobox()" id="tagedit-input" disabled="disabled" class="tagedit-input-disabled" dir="'+options.direction+'"/>';
			html += '</li>';
			html += '</ul>';
				...
				.find('#tagedit-input')
					.each(function() {
						...
						$(this).bind('transformToTag', function(event, id) {
							...
							*/
							//create a new tag -- don't check for duplicates
							html = '<li class="tagedit-listelement tagedit-listelement-old">';
							html += '<span dir="'+options.direction+'">' + $(this).val() + '</span>';
							var name = oldValue? baseName + '['+id+options.addedPostfix+']' : baseName + '[]';
							html += '<input type="hidden" name="'+name+'" value="'+$(this).val()+'" />';
							...
							//do the search
							//alert("about to update search");
							UpdateSearchParameters();


						})
						.keydown(function(event) {
							var code = event.keyCode > 0? event.keyCode : event.whichch;
								...
								case 8: // BACKSPACE
									if($(this).val().length == 0) {
										...
										elementToRemove.fadeOut(options.animSpeed, function() {
											...
											console.log("updating search after backspace");
											UpdateSearchParameters();
					...
					switch(event.target.tagName) {
						case 'A':
							$(event.target).parent().fadeOut(options.animSpeed, function() {
								...
								console.log("updating search after click to delete");
								UpdateSearchParameters();
								});
							break;
						case 'INPUT':
		...
		*/
		function doEdit(event) {
			options.allowEdit = false; //i'm hardcoding this for now.
		...
		function isNew(value, checkAutocomplete) {
			...
			if (isNew == true && checkAutocomplete == true && options.autocompleteOptions.source != false) {
				...
				for (var i = 0; i < result.length; i++) {
                    var label = options.checkNewEntriesCaseSensitive == true? result[i] : result[i].toLowerCase();
